...
export class WebhookService {
  ...
  async processWebhook(webhookData: WebhookDto) {
    ...
    // Extract event and data from webhook payload
    ...
    try {
      // Try to find transaction by Flint transaction ID first
      let transaction: any = null;

      if (transactionId) {
        // Search based on determined transaction type
      ...
      // If not found by transaction ID, try by reference
