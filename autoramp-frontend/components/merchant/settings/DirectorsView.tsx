...
import { FileText, CheckCircle2, User, Edit2, Trash2, Plus, X, Loader2, Upload } from "lucide-react";
...
import { useMerchantStatus, useMerchantDirectors } from "@/lib/hooks";
...
const EditForm = ({
    ...
    <div className="space-y-6 bg-white/5 p-6 rounded-2xl border border-white/10 animate-in slide-in-from-top duration-300">
        <div className="flex justify-between items-center mb-4">
            <h4 className="text-lg font-bold">{isAdding ? "Add New Director" : "Edit Director"}</h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
                ...
                <Input name="role" value={formState.role || ""} onChange={handleInputChange} className="bg-black/40 border-white/10" />
            </div>
            <div className="space-y-2">
...
export default function DirectorsView() {
    ...
    // Granular fetch for directors
    const { data: directorsData, isLoading, refetch } = useMerchantDirectors(merchantId || undefined);
    ...
    const [formState, setFormState] = useState<Director>({});

    const startEditing = (director: Director) => {
        setEditingId(director.id || null);
        setFormState({ ...director, role: director.metadata?.role || director.role });
        setIsAdding(false);
    };
    ...
    const handleSave = async () => {
        if (!formState.firstName || !formState.lastName || !formState.nationality || !formState.bvn || !formState.role || !formState.idType || !formState.idUrl || !formState.proofOfAddress) {
        ...
        setIsSaving(true);
        try {
            // Remove role from root payload as it belongs in metadata
            const { role, ...rest } = formState;
            const payload = {
                ...rest,
                merchantId: merchantId,
                metadata: { ...formState.metadata, role: role }
            };

            if (isAdding) {
                await merchantApi.submitDirector(payload);
                toast({ title: "Director Added", variant: "success" });
            } else if (editingId) {
                await merchantApi.updateDirector(editingId, payload);
                toast({ title: "Director Updated", variant: "success" });
            }
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await merchantApi.removeDirector(id);
            toast({ title: "Director Removed", variant: "success" });
            refetch();
        } catch (error) {
    ...
    if (isLoading && directors.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-20 space-y-4">
                ...
                <p className="text-muted-foreground animate-pulse">Loading directors details...</p>
            </div>
        );
    }
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold flex items-center gap-2">
                    <User className="text-primary" /> Company Directors
                </h3>
                {!isAdding && !editingId && (
            ...
            <div className="space-y-12">
                {directors.map((director: Director, index: number) => (
                    <div key={director.id || index} className={index !== 0 ? "pt-12 border-t border-white/10" : ""}>
                            ...
                            <>
                                <div className="flex items-center justify-between mb-8">
                                    ...
                                    <div className="flex gap-2">
                                        <Button variant="ghost" size="sm" onClick={() => startEditing(director)} className="hover:bg-primary/10 text-primary">
                                            <Edit2 className="w-4 h-4 mr-2" /> Edit
                                        </Button>
                                        <Button variant="ghost" size="sm" onClick={() => director.id && handleDelete(director.id)} className="hover:bg-red-500/10 text-red-500">
