...
export function OffRampTable({ referenceSearchTerm }: OffRampTableProps) {
  ...
  const filteredTransactions = offRampTransactionsData?.data?.transactions || [];
        ...
        <table className="min-w-full divide-y divide-border">
          <thead className="bg-muted">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Bank Code</th>
            ...
            {filteredTransactions.length > 0 ? (
              filteredTransactions.map((txn, index) => (
                <tr key={txn.id} onClick={() => handleRowClick(txn)} className="cursor-pointer hover:bg-muted-foreground/10">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">{txn.amount_offramp?.toLocaleString()}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">{txn.fiatAmount?.toLocaleString()}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">{txn.bankCode}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">{txn.accountNumber}</td>
                </tr>
              ))
            ) : (
              <tr>
